...
func registerForUpdates(stationName, infoURL string, updates chan models.TrackUpdate) tea.Cmd {
	return func() tea.Msg {
		interrupt := make(chan os.Signal, 1)
		signal.Notify(interrupt, os.Interrupt)

		c, _, err := websocket.DefaultDialer.Dial(infoURL, nil)
		if err != nil {
		...
		go func() {
			...
			for {
				...
				update := models.TrackUpdate{StationName: stationName}
				update.Info, err = parseTrackInfo([]byte(message))
				if err != nil {
